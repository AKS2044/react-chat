...
const Header: React.FC<HeaderProps> = (props) => {
  ...
  const onClickLeaveChat = async () => {
    ...
    if (checkAlert) {
      await dispatch(
        fetchLeaveTheChat({ userId: props.userId, chatId: props.chatId })
      );
      navigate("/profile");
    }
  };
    ...
    <div className={cl.header}>
      ...
      <div className={cl.header__title}>
        <div className={cl.header__title__welcome}>
          Welcome,
          <Link to="/profile" className={cl.header__title__welcome__profile}>
            {props.userName}
          </Link>
          to {props.chatName}
        </div>
        <img
          src={logout}
          onClick={() => onClickLeaveChat()}
          alt="Leave the chat"
          title="Leave the chat"
          className={cl.header__title__logout}
        />
      </div>
    </div>
  );
