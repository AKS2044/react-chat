...
const Register = () => {
    ...
    const { 
        register, 
        handleSubmit, 
        formState: {errors, isValid}} = useForm<RegisterParams>({
        mode: 'onBlur'
    });
    ...
    return (
        <div className={cl.register}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className={cl.register__form}>
                ...
                {errors.userName?.message && <span className={cl.register__error}>{errors.userName?.message}</span>}
                <input 
                placeholder='E-mail' 
                type='email'
                className={cl.input}
                {...register('email', {required: 'enter an email'})} />
                {errors.email?.message && <span className={cl.register__error}>{errors.email?.message}</span>}
                <input 
                ...
                {errors.password?.message && <span className={cl.register__error}>{errors.password?.message}</span>}
                <input 
                ...
                {errors.passwordConfirm?.message && <span className={cl.register__error}>{errors.passwordConfirm?.message}</span>}
                <input
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChangeFile(e)}
                type='file'
                accept='image/*, .png, .jpg, .web'
                />
                <Button type={TypeButton.submit} disabled={isValid}>Register</Button>
                <div className={cl.register__form__reg}>Log in: <Link to='/login' className={cl.register__form__reg__link}>Here</Link></div>
            </form>
        </div>
    );
};
