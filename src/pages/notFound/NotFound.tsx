...
const NotFound = () => {
    ...
    return (
        <div className={cl.notfound}>
            ...
            <div className={cl.notfound__text}>{serverError.message ? serverError.message : 'Page is not found'} :(</div>
            <div className={cl.notfound__404}>{serverError.status ? serverError.status : 404}</div>
        </div>
    );
};
